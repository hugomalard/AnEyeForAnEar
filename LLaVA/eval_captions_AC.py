...
parser.add_argument("--use_prefix", type=str2bool, default=False)
...
tokenizer, model, image_processor, context_len = load_pretrained_model(
    model_base, None,model_name,audio='laion/clap-htsat-unfused',modality=args.modality,prefix=args.use_prefix
)
...
#model_path = '../expe/ATOM/models/best_audio_model.pth' 
...
#load mlp
if args.use_mlp:
    path_mlp=args.model_path+'/prefixMLP.bin' 
    mlp_weight = torch.load(path_mlp, map_location=device)
    a,b=model.load_state_dict(mlp_weight,strict=False)
...
df = pd.read_csv('/tsi/dcase/AudioCaps/metadata.csv') #path to AudioCaps metadata file
df = df[df['set']=='test']
...
for i, (audios,images,captions) in enumerate(file_path_dataloader):
    captions = [captions[i][0] for i in range(len(captions))]
    ACD_captions.append(captions)
...
gen_captions = [s.replace('\n', '') for s in gen_captions]
from aac_metrics import evaluate
corpus_scores, rec = evaluate(gen_captions,ACD_captions)
print(corpus_scores)