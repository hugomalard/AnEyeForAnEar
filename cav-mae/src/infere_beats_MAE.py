...
sys.path.append('/home/ids/hmalard/beatscustom')
from beatscustom.base import load_model
...
parser.add_argument("--precise_frame", type=ast.literal_eval, help="use random frames or chosen ones")
...
audio_conf = {'num_mel_bins': 128, 'target_length': args.target_length, 'freqm': 0, 'timem': 0, 'mixup': args.mixup, 'dataset': args.dataset, 'mode':'train', 'mean':args.dataset_mean, 'std':args.dataset_std,
              'noise':args.noise, 'label_smooth': 0, 'im_res': im_res, 'frame_use': 5,'idsFile':'/tsi/audiosig/audible/dcase/data/AudioSet/metadata.csv'}
val_audio_conf = {'num_mel_bins': 128, 'target_length': args.target_length, 'freqm': 0, 'timem': 0, 'mixup': 0, 'dataset': args.dataset,
                  'mode':'eval', 'mean': args.dataset_mean, 'std': args.dataset_std, 'noise': False, 'im_res': im_res, 'frame_use': 5}
...
if args.bal == 'bal':
    ...
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf,return_id = True,beats=True),
        batch_size=args.batch_size, sampler=sampler, num_workers=args.num_workers, pin_memory=True, drop_last=True)
else:
    print('balanced sampler is not used')
    print(args.precise_frame)
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf,precise_frame=args.precise_frame,return_id = True,beats=True),
...
if args.model == 'cav-mae':
    ...
    vision_model = models.CAVMAE(audio_length=args.target_length, norm_pix_loss=args.norm_pix_loss, modality_specific_depth=11, tr_pos=args.tr_pos)
else:
...
if args.pretrain_path != 'None':
    ...
    if not isinstance(vision_model, torch.nn.DataParallel):
        vision_model = torch.nn.DataParallel(vision_model)
    miss, unexpected = vision_model.load_state_dict(mdl_weight, strict=False)
...
def save_feat(a,v,ids,i):
    torch.save(a,'/tsi/audiosig/audible/dcase/data/AudioSet_Frames_CAV/Beats_Feats/'+str(i)+'.pt')
    torch.save(v,'/tsi/audiosig/audible/dcase/data/AudioSet_Frames_CAV/MAE_Feats/'+str(i)+'.pt')
    torch.save(ids,'/tsi/audiosig/audible/dcase/data/AudioSet_Frames_CAV/ids_Feats/'+'ids'+str(i)+'.pt')


import torch.nn as nn
from torch.cuda.amp import autocast,GradScaler

def infere(audio_model, vision_model,val_loader, args):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    vision_model.blocks_a = None
    if not isinstance(audio_model, nn.DataParallel):
        audio_model = nn.DataParallel(audio_model)
    if not isinstance(vision_model, nn.DataParallel):
    ...
    #remove audio part of the CAV MAE
    vision_model = vision_model.to(device)
    vision_model.eval()
    print('starting loop')
    with torch.no_grad():
        for i, (a_input, v_input, index) in enumerate(val_loader):
            a_input = a_input.to(device)
            v_input = v_input.to(device)
        
            with autocast():
                v = vision_model(a_input,v_input,return_vis=True)
                a_input = (a_input - fbank_mean) / (2 * fbank_std)
                a,_ = audio_model(a_input, layer=-1,spec=True)
            save_feat(a,v,index,i)
            print(i)
...
infere(audio_model, vision_model,train_loader, args)