...
def extract_frame(input_video_path,df, target_fold, extract_frame_num=10):
    ...
    rel_path = input_video_path.split('/')[-2:]
    rel_path = os.path.join(*rel_path)
    video_id = df[df['video_file_path']==rel_path]['file_id'].values[0]
...
if __name__ == "__main__":
    ...
    input_filelist = input_filelist[(part-1)*int(n/10):part*int(n/10)]
    ...
    for file_id in range(num_file):
        try:
            ...
            extract_frame(input_filelist[file_id],AS, args.target_fold)
        except:
