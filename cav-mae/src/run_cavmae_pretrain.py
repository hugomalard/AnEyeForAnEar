...
if args.bal == 'bal':
    ...
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf,precise_frame=args.precise_frame,use_clip=args.use_clip,use_clap=args.use_clap),
        batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

val_loader = torch.utils.data.DataLoader(
    dataloader.AudiosetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf,use_clip=args.use_clip),
...
if args.model == 'cav-mae':
    ...
    audio_model = models.CAVMAE(audio_length=args.target_length, norm_pix_loss=args.norm_pix_loss, modality_specific_depth=11, tr_pos=args.tr_pos,use_clip=args.use_clip,use_clap=args.use_clap)
else:
...
if args.use_clip:
    ...
    if args.use_clap:
        audio_model.module.blocks_a = None
    audio_model.module.decoder_blocks = None
    audio_model.module.requires_grad_(False)
    audio_model.module.proj.requires_grad_(True)
    audio_model.module.audio2clip.requires_grad_(True)
    for name, param in audio_model.module.named_parameters():
        if param.requires_grad:
            print(f'{name}: requires gradients')
    lastCKPT = torch.load('/home/ids/hmalard/CAV-CLIP/cosineContrast/models/best_audio_model.pth',map_location='cpu')
    m,u = audio_model.load_state_dict(lastCKPT,strict =False)
    print(m, u)
    print('loaded')
