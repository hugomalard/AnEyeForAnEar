...
import models
import numpy as np
from traintest_distribution_alignment import train
...
parser.add_argument("--quantizer", type=str, default="", help="directory of the quantizer")
...
parser.add_argument("--use_clip", help='use clip or not', type=ast.literal_eval)
parser.add_argument("--use_clap", help='use clap or not', type=ast.literal_eval)
parser.add_argument("--x_attn", help='use cross attention or not', type=ast.literal_eval)
#parser.add_argument("--use_mmd", help='use mmd or not', type=ast.literal_eval)
parser.add_argument("--method", type=str, default='mmd', help="alignment method", choices=['mmd', 'emd'])
...
audio_conf = {'num_mel_bins': 128, 'target_length': args.target_length, 'freqm': 0, 'timem': 0, 'mixup': args.mixup, 'dataset': args.dataset, 'mode':'train', 'mean':args.dataset_mean, 'std':args.dataset_std,
              'noise':args.noise, 'label_smooth': 0, 'im_res': im_res, 'frame_use': 0}
...
if args.bal == 'bal':
    ...
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf,
        precise_frame=args.precise_frame,use_clip=args.use_clip,use_clap=args.use_clap,img2img=False,return_id=True,ar=False),
        batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

val_loader = torch.utils.data.DataLoader(
    dataloader.AudiosetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf,use_clip=args.use_clip,img2img=False,ar=False,use_clap=args.use_clap),
...
if args.model == 'cav-mae':
    ...
    audio_model = models.CAVMAE(audio_length=args.target_length, norm_pix_loss=args.norm_pix_loss, 
                                modality_specific_depth=11, tr_pos=args.tr_pos,use_clip=args.use_clip,
                                dali=True,method=args.method,x_attn=args.x_attn)
else:
...
if args.use_clip:
    audio_model.module.blocks_v = None
    audio_model.module.decoder_blocks = None
    audio_model.module.decoder = None
    audio_model.module.requires_grad_(True)
    audio_model.module.vision_tower.requires_grad_(False)
    audio_model.module.proj.requires_grad_(True)

    for name, param in audio_model.module.named_parameters():
