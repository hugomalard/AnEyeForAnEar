...
def train(audio_model, train_loader, test_loader, args):
    ...
    print('Cross attention :',args.x_attn)
    ...
    if args.x_attn:
        trainablesX = [p for p in audio_model.module.audio_scorer.parameters()]
        trainablesX.extend(p for p in audio_model.module.image_scorer.parameters())
        trainablesX.extend(p for p in audio_model.module.a_q.parameters())
        trainablesX.extend(p for p in audio_model.module.a_k.parameters())
        trainablesX.extend(p for p in audio_model.module.a_v.parameters())
        trainablesX.extend(p for p in audio_model.module.i_k.parameters())
        trainablesX.extend(p for p in audio_model.module.i_q.parameters())
        trainablesX.extend(p for p in audio_model.module.i_v.parameters())
        trainables = list(set(trainables)-set(trainablesX))
        optimizerX = torch.optim.Adam(trainablesX, args.lr, weight_decay=5e-7, betas=(0.95, 0.999))
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, ind) in enumerate(train_loader):
            B = a_input.size(0)
            ...
            if args.x_attn:
                ...
                if epoch > 1 and epoch <6:
                    lossX =  loss- loss_mae *100*(6-epoch+1)
                    scalerX.scale(lossX).backward(retain_graph=True)
                    scalerX.step(optimizerX)
                    scalerX.update()
                elif epoch >= 6:
                    lossX =  loss- loss_mae *100
                    scalerX.scale(lossX).backward(retain_graph=True)
                    scalerX.step(optimizerX)
                    scalerX.update()
...
def validate(audio_model, val_loader, args):
    ...
    with torch.no_grad():
        for i, (a_input, v_input, _) in enumerate(val_loader):
            ...
            with autocast():
                loss, loss_mae, loss_mae_a, loss_mae_v, loss_c, mask_a, mask_v, c_acc = audio_model(a_input, v_input, args.masking_ratio, args.masking_ratio, mae_loss_weight=args.mae_loss_weight, contrast_loss_weight=args.contrast_loss_weight, mask_mode=args.mask_mode,evaluate=True)
                loss, loss_mae, loss_mae_a, loss_mae_v, loss_c, c_acc = loss.sum(), loss_mae.sum(), loss_mae_a.sum(), loss_mae_v.sum(), loss_c.sum(), c_acc.mean()
