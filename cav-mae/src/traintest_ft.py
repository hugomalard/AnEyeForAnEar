...
def train(audio_model, train_loader, test_loader, args):
    ...
    audio_model.train()
    if args.start_linear == 1:
        print('Pretrained backbone parameters are frozen for first epoch.')
        for param in base_params:
            param.requires_grad = False
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, labels) in enumerate(train_loader):
            ...
            lossL.append(loss.to('cpu').detach())
        ...
        result[epoch-1, :] = [np.mean(lossL),valid_loss,acc, mAP, mAUC, optimizer.param_groups[0]['lr']]
